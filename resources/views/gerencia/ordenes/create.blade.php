...
$saveUrl = isset($cotizacion) ? route('ordenes.guardarDesdeCotizacion') : route('ordenes.store');

$costoServicioPrefill = old('precio',
    isset($cotizacion)
        ? (data_get($cotizacion, 'servicio.precio') ?? ($cotizacion->precio_servicio ?? 0))
        : 0
);

$costoOperativoPrefill = old('costo_operativo',
    isset($cotizacion)
        ? ($cotizacion->costo_operativo ?? data_get($cotizacion, 'servicio.costo_operativo') ?? 0)
        : 0
);

$descripcionServicioPrefill = isset($cotizacion)
...
<div class="max-w-7xl mx-auto"
     x-data="formOrdenServicio(
        @js($productosPrefill ?? []),
        @js(data_get($cotizacion ?? null, 'moneda') ?? old('moneda','MXN')),
    ...
    <div class="relative mb-10 text-center mx-a">
        <h1 class="text-2xl md:text-3xl font-bold text-gray-800">Nueva Orden de Servicio</h1>
    ...
    <form id="ordenForm"
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6 mb-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div>
                    ...
                    <select name="tipo_orden" x-model="tipoOrden" class="w-full rounded-lg border-gray-300 focus:ring-2 focus:ring-blue-500" required>
                        @foreach($tiposOrdenOpts as $t)
                            <option value="{{ $t }}" @selected(old('tipo_orden')===$t)>{{ $labels[$t] ?? ucfirst(str_replace('_',' ', $t)) }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div>
                    ...
                    <select name="prioridad" class="w-full rounded-lg border-gray-300 focus:ring-2 focus:ring-blue-500" required>
                        @foreach(($prioridades ?? ['Baja','Media','Alta','Urgente']) as $p)
                            <option value="{{ $p }}" @selected(old('prioridad')===$p)>{{ $p }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="md:col-span-3">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            ...
                            @php $oldTec = collect(old('tecnicos_ids', []))->map(fn($v)=>(int)$v); @endphp
                        ...
                        <div>
                            ...
                            <select name="id_tecnico"
                                ...
                                @foreach(($tecnicos ?? []) as $t)
                                    <option value="{{ $t->id }}" @selected(old('id_tecnico')==$t->id)>{{ $t->name }}</option>
                                @endforeach
                            </select>
                ...
                <div>
                    ...
                    <input type="text" name="estado" value="{{ old('estado','Pendiente') }}" class="w-full rounded-lg border-gray-300 focus:ring-2 focus:ring-blue-500">
                </div>
            ...
            <div class="mt-4">
                ...
                <textarea name="servicio" rows="2" class="w-full rounded-lg border-gray-300 focus:ring-2 focus:ring-blue-500" placeholder="Ej. Mantenimiento preventivo a sistema X">{{ old('servicio', data_get($cotizacion ?? null, 'servicio.nombre', '')) }}</textarea>
            </div>
            ...
            <div class="mt-4">
                ...
                <textarea name="descripcion" rows="3" class="w-full rounded-lg border-gray-300 focus:ring-2 focus:ring-blue-500" placeholder="Notas generales de la orden">{{ old('descripcion', data_get($cotizacion ?? null, 'descripcion')) }}</textarea>
            </div>
        </div>
        ...
        <div class="mt-4">
            ...
            <textarea name="condiciones_generales" rows="3"
                      class="w-full rounded-lg border-gray-300 focus:ring-2 focus:ring-blue-500"
                      placeholder="Notas internas para el equipo">{{ old('condiciones_generales') }}</textarea>
        </div>
    ...
    <div id="pdfPreviewModal" class="fixed inset-0 z-50 hidden">
        ...
        <div class="relative mx-auto my-2 sm:my-6 bg-white rounded-xl shadow-xl max-w-5xl w-[calc(100vw-1rem)] sm:w-[95vw] h-[94vh] sm:h-[85vh] flex flex-col overflow-hidden">
            <div class="px-4 py-3 border-b flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
                <h3 class="font-semibold text-gray-800 leading-tight">Previsualización de la orden (PDF)</h3>
...
(function(){
  ...
  async function releaseSerials(token, codigoProducto=null, series=null){
    ...
    try {
      await postJson(LIBERAR_SERIES_URL, payload);
    } catch(e) {}
  }
