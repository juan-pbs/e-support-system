...
<div class="max-w-7xl mx-auto">
    @if (session('success'))
        <div id="alert-success" class="mb-4 bg-green-100 border border-green-400 text-green-700 px-4 py-3 rounded"> {{ session('success') }} </div>
    ...
    <form action="{{ route('proveedores.actualizar', $proveedor->clave_proveedor) }}" method="POST" class="bg-white border border-gray-200 shadow-xl rounded-xl p-6 space-y-5"
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
                ...
                <input type="text" name="nombre" value="{{ old('nombre', $proveedor->nombre) }}" required class="w-full border rounded-lg px-4 py-3">
                @error('nombre') <p class="text-red-600 text-sm mt-1">{{ $message }}</p> @enderror
            </div>

            <div>
                <label class="block text-sm font-medium text-gray-700 mb-2">RFC</label>
                <input type="text" name="rfc" value="{{ old('rfc', $proveedor->rfc) }}" required class="w-full border rounded-lg px-4 py-3">
            ...
            <div>
                ...
                <input type="text" name="alias" value="{{ old('alias', $proveedor->alias) }}" class="w-full border rounded-lg px-4 py-3">
            ...
            <div>
                ...
                <input type="email" name="correo" value="{{ old('correo', $proveedor->correo) }}" class="w-full border rounded-lg px-4 py-3">
                @error('correo') <p class="text-red-600 text-sm mt-1">{{ $message }}</p> @enderror
            </div>

            <div>
                <label class="block text-sm font-medium text-gray-700 mb-2">Teléfono</label>
                <input type="tel" name="telefono" value="{{ old('telefono', $proveedor->telefono) }}" pattern="[0-9]{7,20}" title="Solo números (7 a 20 dígitos)" oninput="this.value=this.value.replace(/[^0-9]/g,'')" required class="w-full border rounded-lg px-4 py-3">
            ...
            <div>
                ...
                <input type="text" name="contacto" value="{{ old('contacto', $proveedor->contacto) }}" class="w-full border rounded-lg px-4 py-3">
            ...
            <div class="sm:col-span-2 rounded-2xl border border-slate-200 bg-slate-50 p-4">
                <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                    <div>
                        ...
                        <p class="text-xs text-gray-500">Usa una dirección validada para recolecciones programadas.</p>
                    </div>
                ...
                <div class="mt-4 grid grid-cols-1 gap-4 md:grid-cols-2">
                    <div class="md:col-span-2">
                        ...
                        <input type="text" x-model="direccion_formateada"
                            ...
                            readonly
                            @click="openPicker()">
                        <p class="mt-1 text-xs text-gray-500">La dirección se edita desde el mapa para mantener ubicación y validación correctas.</p>
