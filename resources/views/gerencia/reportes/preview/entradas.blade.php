...
<div x-show="f.tipo === 'entradas'" x-cloak class="space-y-6">
                            ...
                            <div class="border-t border-dashed border-slate-200 pt-3">
                                <div class="overflow-x-auto">
                                    <div class="min-w-[1100px] grid gap-2 text-[11px]"
                                        ...
                                        {{-- Primeras 8 filas --}}
                                        <template x-for="(row, ri) in tabla.rows.slice(0, 8)" :key="'ent-pdf-r-'+ri">
                                            <template x-for="(col, ci) in tabla.cols" :key="'ent-pdf-c-'+ri+'-'+ci">
                                                <div
                ...
                <div class="px-5 pt-4 pb-3 border-b border-slate-100 flex items-center justify-between">
                    <div>
                        ...
                        <div class="mt-1 text-sm font-semibold text-slate-800 flex items-center gap-1.5">
                            ...
                            Plantilla de entradas de inventario
                        </div>
                    </div>
                        ...
                        <div class="overflow-x-auto">
                            <table class="min-w-full text-[11px] sm:text-xs">
                                <thead class="bg-emerald-50 border-b border-emerald-100">
                                    <tr>
                                        <template x-for="(col, ci) in tabla.cols" :key="'ent-xl-h-'+ci">
                                            <th
                                ...
                                <tbody>
                                    {{-- Primeras 10 filas --}}
                                    <template x-for="(row, ri) in tabla.rows.slice(0, 10)" :key="'ent-xl-r-'+ri">
                                        <tr class="odd:bg-white even:bg-slate-50/80">
                                            <template x-for="(col, ci) in tabla.cols" :key="'ent-xl-c-'+ri+'-'+ci">
                                                <td
    ...
    {{-- Cuando NO hay datos de entradas --}}
    <template x-if="!tabla.rows.length">
